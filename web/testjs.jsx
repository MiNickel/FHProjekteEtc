...
if (typeof(Storage) !== "undefined"){
    ...
    console.log("Sorry. LocalStorage wird nicht unterst�zt");
}
*/
...
if (typeof(Storage) !== "undefined"){
    ...
    if (visits) {
        ...
        */
       console.log("Laden der contentjson");
        fetch('ContentJSON.json').then(
		function(response) {
		...
		function(jsonData) {
                    for(i=0; i<jsonData.length ; i++){
                        let str = jsonData[i];  
                        localStorage.setItem(i, ArtikelToJSON(str));
                        artikel[i] = ArtikelToJSON(str);
...
function saveArticle(){
    ...
    if(document.getElementById("newsButton").checked){
        ...
        artikel = new News(inhalt, titel, farbe, startdatum, enddatum, kategorie, quelle);
        let storageLength = localStorage.length;
        let newPos = storageLength;
        let str = ArtikelToJSON(artikel);
        localStorage.setItem(newPos, str);
        ...
        artikel = new Projekt(inhalt, titel, farbe, startdatum, enddatum, professor, anzahlFreiePlaetze);
        let storageLength = localStorage.length;
        localStorage.setItem(storageLength, ArtikelToJSON(artikel));
        ...
        artikel = new Aufgabe(inhalt, titel, farbe, startdatum, enddatum, fach);
        let storageLength = localStorage.length;
        localStorage.setItem(storageLength, ArtikelToJSON(artikel));
    }
    /*console.log(artikel);
    postData('ContentJSON.json', artikel).then(
		function(data) {
			console.log(data);
    ...
    (async ()=> {
        const rawResponse = await fetch('ContentJSON.json', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
