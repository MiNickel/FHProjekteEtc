...
void main()
{
	...
	vec3 diffuse = vec3(1.0, 0.0, 0.0) * material.x * max(0.0, dot(n, light)); //ausbreitung-diffuse reflexion
	vec3 specular = vec3(1.0, 0.0, 0.0) * material.y * pow(max(0.0, dot(reflectionVec, viewVector)), shine); //Spiegelnde reflexion
	vec3 ambient = vec3(1.0, 0.0, 0.0) * material.z * ambientLight;	//Umgebung
